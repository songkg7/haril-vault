...
[[Java]] 는 [[운영체제 (Operating system)|OS]] 가 직접 실행할 수 있는 언어는 아니다. 컴퓨터가 실행하기 위해 기계어로 변환과정이 필요한데 이를 컴파일(Compile)이라고 한다. 컴파일 과정이 필요한 언어를 컴파일 언어라고 하며, 컴파일 과정의 유무는 언어의 성능을 가늠하는 중요 포인트 중 하나이다. 컴파일 언어들은 이 컴파일 과정을 최적화하기 위해 각고의 노력을 다하고 있다. 자바 또한 대표적인 컴파일 언어다. 자바에서 컴파일은 어떻게 동작하는지 한 번 살펴보자.

...
tags:
  ...
  - version-manager
categories: 
updated: 2024-06-25 14:32:20 +0900
...
지금까지 이 분야에서 가장 유명한건 [[asdf]] 였어요. 하지만 최근 mise 를 사용하기 시작한 뒤로는 mise 가 UX 측면에서 조금 더 괜찮다는 생각이 들었어요. 이번 글에서는 간단한 사용 용례를 소개해드려보려고 해요.
...
새로운 환경으로 취직 or 이직하게 되서 새로운 장비를 받았다고 가정해볼까요? 새로운 곳에서는 [[Java]] 를 기본으로 사용하고 있으니, 필요로하는 버전을 설치할 수 있는지 확인해봅니다.
...
mise 를 사용할 때는 `.mise.toml` 이라는 파일을 사용해서 어떤 버전을 사용할 것인지 명시합니다.[^1]
...
> [!tip]
> 그동안 [[asdf]] 를 사용해와서 `.tool-versions` 라는 파일이 이미 있다면, `tool-versions` 라는 파일을 그대로 사용할 수도 있습니다.

이제 프로젝트 별로 다른 Java 버전을 사용할 수 있게 되었습니다.[^2] 어떤 Java 버전을 사용하는지 `.mise.toml` 파일에 명시되어 있으므로, 팀원들간 개발환경 차이로 인해 발생하는 문제도 최소화할 수 있을 것 같습니다.

생각해보니 Java 11 은 이제 없어도 될 것 같네요. 그냥 17을 기본값으로 쓰고, 11 은 삭제하겠습니다.
...
자, 이제 업무 환경 설정이 끝났으니 개인적으로 자주 사용하는 언어들을 좀 더 설치해야겠네요.

저는 옵시디언의 플러그인인 [[o2|O2]] 를 [[TypeScript]] 를 사용하여 개발 중이에요.

```bash
mise use node@lts
```

[[Python]] 기반인 [[Anaconda]] 를 사용해야하는 일이 종종 있기도 합니다.

```bash
mise use python@3.12.3 # 파이썬은 버전에 민감하여 최대한 구체적인 버전을 사용하고 있어요.
```

업무용 메신저 봇을 [[Go]] lang 으로 만들기도 하구요. 트래픽 실험에 사용하는 API 도 golang 으로 만들고 있어요.

```bash
mise use go@latest
```

~~Rust, Ruby 등등을 간간히 사용하지만 생략...~~
...
이렇게 대략적인 개발환경 구성을 끝냈습니다. 다들 새로운 장비를 받고 개발 환경을 구성하는데 1~2시간은 걸린다고 생각하실테니 남는 시간은 개발 서적이나 읽어야겠습니다. 언어만 설치했는데 뭐가 끝이냐구요? 다른 부분은 [dotfile 관리하기](https://haril.dev/blog/2023/03/26/chezmoi-awesome-dotfile-manager) 를 참고해주세요.
...
[^2]: 예시로 Java 를 사용했을 뿐이고, Java 는 [[IntelliJ]] 에서 설정하는걸 더 선호합니다.

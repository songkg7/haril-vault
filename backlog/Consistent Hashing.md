...
categories: 
updated: 2023-06-01 16:40:52 +0900
banner: "![[consistent hash.png]]"
---

안정 해시를 설명하기 전에 먼저 해시([[Hash]])에 대해서 간단하게 되집어보고 넘어가 봅니다. 글이 다소 길기 때문에 편의상 '~한다.' 체로 설명합니다.
...
안정 해시를 살펴보기 전에 먼저 왜 안정 해시가 등장하게 되었는지를 이해하기 위해 기존 해시 라우팅 방식을 살펴보자.
...
안정 해시는 위에서 살펴본 기존 해시 방식의 문제점을 해결하기 위해 등장했다.

%%현실 세계에서 노드의 변동은 잦은 일에 속한다. 네트워크 오류로 일시적인 노드 이탈이 발생할 수도 있고, 트래픽의 증가에 대응하기 위해 노드를 추가해야할 수도 있다. 이렇게 노드 목록이 유동적인 상태를 유지해야하는 경우, 노스 숫자로 해시하는 단순한 방식으로는 서버를 운영하기 어렵다.%%

![[Pasted image 20230601132426.png]]

아이디어는 간단하다. 해시 공간의 시작과 끝을 이어서 일종의 고리(ring)을 만든 후, 해시 공간 위에 노드들을 배치한다.
...
해시 함수를 통과한 트래픽은 링 위에서 가장 가까운 노드를 향해 라우팅된다. B 노드는 이후 같은 요청이 들어올 때를 대비해서 `key1` 을 캐시 해둘 수 있겠다.
...
이전에 들어왔던 키는 항상 해시 링 위의 같은 지점에 배치되기 때문에 D 와 C 노드 사이에 배치되던 키 중 일부는 새로운 E 노드로 향하기 때문에 캐시가 미스하게 되지만, 나머지 노드들은 캐시 미스가 발생하지 않는다.

네트워크에 에러가 발생하여 C 노드가 사라진 상황을 가정해도 결과는 비슷하다.

![[Pasted image 20230601133814.png]]

C 노드로 향할 키들이 D 노드로 향하여 캐시 미스가 발생하지만 그 외 공간에 배치된 키들은 캐시 미스가 발생하지 않는다.

결론적으로, 노드 목록에 어떠한 변화가 생기던지간에 변경된 노드와 직접적으로 관련된 키들만 캐시 미스가 발생하기 때문에 수평 확장에 비교적 자유롭게 대처할 수 있다.
...
해시 함수의 결과를 예측하는 것은 어려운 일이다. 그러므로 해시 결과에 의해 링 위 포지션이 정해지는 안정 해시는 노드가 균등한 해시 공간을 가지고 링 위에 분포할 것이라고 보장할 수 없다.
...
노드가 해시 링 위에 균등한 파티션을 가지고 배치되지 않기 때문에 생기는 문제이다. 운이 없으면 위 예처럼 D 노드가 담당하는 해시 공간이 다른 노드에 비해 과하게 크게 설정될 수 있다. 이 상태는 특정 노드에 트래픽이 쏠리면서 장애를 유발하는, 핫스팟이라고 하는 문제를 야기한다.

## 안정 해시의 단점 해결

해시 공간은 유한하다. 그러므로 노드 수가 굉장히 많다면 표준편차가 감소하여 노드 하나가 없어진다 하더라도 다음 노드에 큰 부하는 가지 않을 것이다. 문제는 **물리 노드의 수는 곧 비용**이라는 점이다.

그래서 물리 노드(Physical node)를 모방하는 가상 노드(Virtual node)를 구현하여 해결한다.

![[Pasted image 20230523204810.png]]

가상 노드는 내부적으로 물리 노드의 해시값을 가리키고 있다. 일종의 분신술(...?)이라고 이해하면 된다. 본체는 따로 존재하면서 복제된 인스턴스들이 해시 링 위에서 트래픽을 기다리고 있다. 트래픽이 가상 노드에 할당되면 내부에 존재하는 실제 노드의 해시 값으로 라우팅되는 원리이다.

## 안정 해시 D.I.Y

개인적으로 어떤 개념을 학습할 때 직접 구현해보는 것만큼 확실한 학습 방법은 없다고 생각한다. 직접 구현해보자.

### Hash 알고리즘 선택

안정 해시를 구현할 때는 적절한 해시 알고리즘을 선택하는 것이 매우 중요하다. 해시 함수의 속도가 성능과 직결되기 때문이다. 일반적으로 널리 사용되는 해시 알고리즘은 MD5 와 SHA-256 이 있다.
...
hash ring 은 `TreeMap` 을 사용하여 구현했다. `TreeMap` 은 저장과 동시에 key(해시값)의 오름차순으로 항상 정렬을 유지하기 때문에 `tailMap(key)` 메서드를 통해서 key(해시값)보다 큰 값들을 찾아올 수 있고, 큰 key 를 찾을 수 없다면 `firstKey` 로 가작 작은 key 와 연결하여 링을 구현한다.

> [!INFO] Why TreeMap?
> `TreeMap` 에 대해서는 이 [링크](https://coding-factory.tistory.com/557)를 참조해주기 바란다.

### 테스트

일반 라우팅 방법에 비해 안정 해시 라우팅은 얼마나 효과적일까? 다음과 같은 상황을 가정해봤다.

- 1M 트래픽을 처리한 이후, 노드 목록에 변화를 주고 다시 같은 트래픽이 들어오는 상황을 가정
- 4대의 물리 노드

간단한 테스트 코드[^1]를 통해서 수치화했고, 이를 그래프로 그려봤다.
...
노드들의 그래프 높이가 비슷한 것을 보아 모든 트래픽이 아주 잘 분산되는 것 또한 확인할 수 있다.
...
> [!NOTE] 그래프에는 Virtual node count 가 1 이라고 나와있는 이유
...
Case 1 과 마찬가지로, 첫 번째 요청에서 캐시가 바로 히트할 수는 없으므로 붉은 그래프가 상승하며 두 번째 요청에서는 100% 캐시 적중하므로 녹색 그래프와 붉은 그래프의 높이가 같게 된다.

하지만 노드마다 그래프의 높이가 다른 것을 확인할 수 있는데, 이게 바로 안정 해시의 단점이였던 **균일하지 않은 파티션으로 인하여 골고루 분산되지 않는 트래픽**을 보여준다.
...
Case 2 와 비교하면 캐시 적중률이 압도적으로 뛰어난 것을 확인할 수 있다. 1번 노드로 향했던 트래픽이 2번 노드로 향하면서 2번 노드에 다소 부하를 주게 되었지만, 3번 노드와 4번 노드는 1번 노드가 이탈했음에도 캐시 적중률에 전혀 변화가 없는 점에서 고무적이다.
...
오! 드디어 뭔가 변화가 보인다. 여전히 균등하게 나눠진 파티션은 아닌 것 같지만 Case 4와 비교했을 때 핫스팟 문제는 다소 해결된 것으로 보이고 캐시 적중률도 준수하다. 가상 노드 10대로는 다소 부족한 것 같으니 충분히 늘려보자.

#### Case 6. Consistent Hash, 1대의 노드 이탈, 100대의 가상 노드

![[Pasted image 20230601160256.png]]

물리 노드당 100대의 가상 노드, 즉 현재 해시 링 위에는 400대의 가상 노드가 존재하고 있다. 확인할 수 있는 부분은 아래와 같다.

- Case 1 과도 견줄 수 있을만큼 트래픽이 균일하게 분산되는 것을 확인할 수 있다.
- 1번 노드가 이탈하더라도 이후 1번 노드로 향할 트래픽이 여러 노드들로 분산되면서 핫스팟 문제를 발생시키지 않는다.
- 1번 노드로 향할 트래픽 외에는 대부분의 트래픽이 캐시에 적중한다.

충분한 가상 노드를 배치하니 나머지 연산을 통해 분산하던 라우팅 방식에 비해 수평 확장에 굉장히 유리해졌다는 것을 확인할 수 있었다.

## 마무리

이후 가상 노드를 10000대까지 늘려보니 편차는 점점 더 줄어들었고 거의 0에 수렴해갈 것입니다. 하지만 가상 노드를 늘리는 것은 해시 링 위에 많은 인스턴스가 존재하는 것이고 따라서 불필요한 오버헤드가 발생할 수 있습니다. 새로운 노드가 추가되거나 삭제될 때마다 해시 링위에서 가상 노드를 찾아서 정리해주는 작업이 필요해지기 때문입니다. 운영 중인 환경에 맞춰서 적절한 수의 가상 노드를 설정하시길 바랍니다.
